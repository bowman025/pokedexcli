...
func (c *Client) GetPokemon(pokemon string) (Pokemon, error) {
	urlValue := baseUrl + "/pokemon/" + pokemon

	if data, ok := c.cache.Get(urlValue); ok {
		pokeRes := Pokemon{}
		err := json.Unmarshal(data, &pokeRes)
		if err != nil {
			return Pokemon{}, fmt.Errorf("error during unmarshal: %v", err)
		}
	...
	if err != nil {
		return Pokemon{}, fmt.Errorf("API error: %v", err)
	}
	...
	if err != nil {
		return Pokemon{}, fmt.Errorf("error reading the request: %v", err)
	}
